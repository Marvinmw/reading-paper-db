...
🛠 Skills 可选启用：
  □ se-literature-synthesis  （论文 corpus → 5 类 gap 的系统化综述）
  □ se-pattern-discovery     （时序 / 主题 EDA）
  □ se-abductive-iteration   （多 iter 的跨 paper 反推）
  □ se-idea-framing          （把 cross idea 升级 framing）
...
| Skill | 何时启用 |
|---|---|
| `se-literature-synthesis` | 要做 source × theme 矩阵 / 5 类 gap 的系统化综述（默认推荐启用）|
| `se-pattern-discovery` | 要 7-cut EDA 跨时段找 pattern（密度 / 主题 / author 网络）|
| `se-abductive-iteration` | 多 paper 反复迭代 H1→H2→Hn 时 |
| `se-idea-framing` | cross idea 升级 framing |
| `se-citation-verification` | 报告引用前核查 |
...
可调用 `se-pattern-discovery` 做 7-cut EDA：
...
跨时段 lookback 更要做 — corpus 内可能漏掉跨周 / 跨月 preprint。每个 new_problem idea **必须**用 `se-literature-search` 真去 arxiv/OpenAlex 搜（不只项目内 corpus）。否则 preprint_risk 标签不可信。
...
- 「30 天」/「过去一周」→ 调整窗口
- 「跳过趋势分析」→ Step 3 skip
- 「启用 lit-synthesis」→ 调用 se-literature-synthesis
