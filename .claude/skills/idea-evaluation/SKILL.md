...
🛠 Skills 可选启用：
  □ se-idea-evaluation       （5d Impact/Novelty/Insight/Feasibility/Clarity 替代评分）
  □ se-reviewer-psychology   （模拟特定 venue 的 reviewer 反应）
  □ se-citation-verification （prior_art 真实性核查）
  □ se-literature-search     （搜索 prior work 验证 novelty）
...
📋 流程：
  ...
  5. Prior-art 检查（grep 关键词 / 可选启用 se-literature-search 真搜）
...
- 用户可指定 `output/reports/<file>.json` 的 `idea_sparks[N]` 或 `research_opportunities[N]`

## 可选 skills

| Skill | 何时启用 |
|---|---|
| `se-idea-evaluation` | 想用 5d 体系做 cross-check（与 FINER 互补）|
| `se-reviewer-psychology` | 想模拟 ICSE/CCS/NeurIPS reviewer 的具体反应 |
| `se-citation-verification` | 想核查 closest_prior_work 引用真实性 |
| `se-literature-search` | 想做 OpenAlex/Crossref/Semantic Scholar 五索引真搜 prior work |
| `se-pattern-discovery` | 想做反 pattern 检查（cherry-picking / hasty generalization）|
...
### 4. Prior-art 检查（**强制真实 web search，warning-only 不 cut**）

- 抓 idea title + one_liner 关键词 + 同义词组
- **必须**用 `se-literature-search` skill 跑 arxiv + OpenAlex（不只本地 corpus）
...
- 「启用 lit-search 核查 novelty」→ 加 se-literature-search 真搜 prior work
