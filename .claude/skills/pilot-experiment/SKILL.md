...
🛠 Skills 可选启用：
  □ se-literature-search   （验证 pilot 结果时交叉比对 prior 数据）
  □ se-pattern-discovery   （样本分析阶段做 7-cut EDA）
...
**协议**：
1. 从公开数据集（如 CodeReview-SE / MBPP / HumanEval）选 10-20 个有标注 ground truth 的样本
2. 调目标模型 API（用户须提供 key；否则用免费 Ollama 本地模型降级）
3. 收集输出，与 ground truth 对比，计算效应大小
