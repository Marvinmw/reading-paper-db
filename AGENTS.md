...
覆盖会议：VLDB / SIGMOD / ICDE（数据库）、OSDI / SOSP / NSDI / SIGCOMM / USENIX ATC / EuroSys（分布式/网络系统）、IEEE S&P / ACM CCS / Financial Cryptography（Web3 安全）

arxiv 分类：默认 cs.DB / cs.DC / cs.NI / cs.CR / cs.PL；`--include-se` 时额外纳入 cs.SE 交叉主题
...
**除 Step 1（爬虫）外，其余全部 skill 化**，见 `.agents/skills/`。
...
| papers.cool | 默认 cs.DB / cs.DC / cs.NI / cs.CR / cs.PL（当日热门） |
| VLDB / SIGMOD / ICDE | 数据库顶会（via DBLP） |
| OSDI / SOSP / NSDI / SIGCOMM / ATC / EuroSys | 分布式/网络系统顶会（via DBLP） |
