...
| `evaluation.expected_findings` | ✅ 具体数字 | "在 3 个真实集群中 p99 latency 降低 ≥ 20%" |
...
| 中英混拼专业术语 | "consensus-layer semantics 范式" | "共识层真正保证什么" |

**例外**：领域核心名词允许保留（SQL / LSM-tree / Raft / BFT / EVM / DeFi / MEV / rollup / zkSNARK）——它们是研究对象本身。

#### 对照

| ❌ 长 + jargon | ✅ One-liner |
|---|---|
| "我们假设主流 learned optimizer 在真实多租户 workload 下发生系统性失效并导致 tail latency 和资源利用率同步退化" | "学习型优化器一遇到混合负载就选错计划" |
| "我们提出一类新攻击面「Cross-chain Oracle Drift」——由跨链桥延迟、预言机更新频率和清算窗口共同触发" | "跨链价格更新慢，会制造新的清算攻击窗口" |
| "我们假设 BFT 节点实现中的配置继承会通过 copy-paste 传播，导致同一类安全错误在多个链上系统复现" | "BFT 配置被抄来抄去，一个错会传到多条链" |
...
我们花 6 个月做 prior 的 +1 延续
  ↓
prior preprint 被某顶会接受（往往就是 VLDB/SIGMOD/SOSP/NSDI/CCS/S&P/FC）
  ↓
...
**例**：一篇 DeFi oracle 论文的 Discussion 如果已经写明 "we do not study cross-chain oracle delay"，那「跨链 oracle 延迟窗口测量」可能只是作者自留 follow-up；必须读完 full-text 后再判断能否标 `new_problem`。
...
1. 真去 **arxiv + OpenAlex / Semantic Scholar / WebSearch** 至少两类来源搜：
   - title 主词
   - 同义词组 / 近邻概念
   - 关键技术词
2. 检查近 **6 个月** preprint
...
**事故模式**：把「DeFi oracle 操纵」或「智能合约 reentrancy 检测」标成 new_problem，但 CCS/S&P/FC 与 arxiv 已有直接系统研究。未做真实 search 就标 new_problem，是结构性失败。

**实操**：每个 new_problem idea 至少跑 1 次 prior-art search，把搜索 query + 结果摘要写入 idea 的 `novelty.search_evidence` 字段（新增可选字段）。
...
| `new_solution` | 已有问题上新方法 + 显著 insight | 自适应事务调度 / 合约静态分析新抽象 |
...
| technical × new_problem | 新型 rollup 数据可用性攻击 / 跨链 oracle drift | 🟢 高 |
...
#### 历史样本
Workload Drift / Consensus Tail Failure / Oracle Poisoning / Cross-chain Delay Windows
...
#### 历史样本
Learned Optimizer 在 workload drift 下失效 / BFT 默认同步假设在真实网络中不成立 / Rollup 成本模型默认数据可用性便宜
...
#### 历史样本
Cross-chain Oracle Drift / MEV-aware Transaction Ordering / Contract Upgrade Shadow State / Rollup Data Availability Failure
...
#### 历史样本
典型 VLDB / SIGMOD / NSDI / CCS technique paper
...
### ❌ 错（claim 重 + 含数值）
```json
"core_claim": "Learned optimizer 在真实多租户负载下系统性失效——3 个系统 p99 latency 上升 ≥ 50%"
```
问题：(1) claim 暗示已有证据；(2) 含数值=伪造证据；(3) jargon 满屏；(4) 多句。

### ❌ 半对（plain 但 50+ 字、多句、还有 jargon）
```json
"formulation": "我们假设 learned query optimizer 在多租户 workload drift 下系统性失效——所有 SOTA learned optimizer 都会产生 tail latency regression"
```
问题：(1) "——" 把它撕成两句；(2) "workload drift""tail latency regression" 是 jargon。

### ✅ 对（one_liner 一句话秒懂）
```json
"idea": {
  "formulation_type": "hypothesis",
  "one_liner": "学习型优化器一遇到混合负载就选错计划",
  "approach": "选 3 个开源查询优化器和 4 类真实混合 workload；固定数据分布后只改变租户组合；比较 learned optimizer 与传统 cost model 的计划选择和 p99 latency"
},
"evaluation": {
  "expected_findings": "若假设成立：混合负载下 learned optimizer 错选计划率 ≥ 30%，p99 latency 比传统 cost model 高 ≥ 20%",
  "null_result": "错选计划率 < 10%，且 p99 latency 差异 < 5%"
}
```

**关键**：one_liner 单句、plain、无数值；技术细节全部下沉到 approach 与 evaluation。
...
- [ ] `is_domain_relevant=true` 或明确解释为何保留跨域 idea；`domain_tags` 至少 1 个
